...
const PsGamesFetch = () => {
  ...
  // Playstation-Plattformkennung
  const psPlatform = "187" && "18" && "16"; // Die genaue Kennung kann variieren, überprüfen Sie die RAWG API-Dokumentation
  ...
  const fetchPageData = async (page, platform) => {
    try {
      ...
      const response = await fetch(
        `https://api.rawg.io/api/games?key=${apiKey}&platforms=${psPlatform}&page=${page}&page_size=${pageSize}&ordering=name`
      );
  ...
  const handlePageChange = async () => {
    ...
    try {
      ...
      const nextPageGames = await fetchPageData(nextPage, psPlatform);
  ...
  const handlePagePrev = async () => {
    ...
    if (prevPage > 0) {
      try {
        ...
        const prevPageGames = await fetchPageData(prevPage, psPlatform);
  ...
  useEffect(() => {
    fetchPageData(currentPage, psPlatform)
  ...
  return (
    <div className="gamesContainer" style={{ display: "flex" }}>
      <p className="titelGames">PlayStation Spiele</p>
